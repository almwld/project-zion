...
class _CosmicDashboardState extends ConsumerState<CosmicDashboard> {
  ...
  Widget _buildKaliAgentCard() {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), gradient: const LinearGradient(colors: [Color(0xFF0A1A0A), Color(0xFF050805)]), border: Border.all(color: (_siActive ? Colors.green : Colors.red).withOpacity(0.5))), child: Row(children: [Icon(_siActive ? Icons.shield : Icons.shield_outlined, color: _siActive ? Colors.green : Colors.red, size: 40), const SizedBox(width: 12), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_siActive ? 'Si - محارب Kali 🗡️' : 'Si - خامل 💤', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), Text(_siActive ? '600+ أداة Kali جاهزة للهجوم المتوازي.' : 'اضغط "إيقاظ" لتفعيل المحارب.', style: const TextStyle(color: Colors.grey, fontSize: 12))])]));
  }
